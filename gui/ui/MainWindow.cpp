...
//    QGraphicsDropShadowEffect* dse = new QGraphicsDropShadowEffect(ui->top);
//    dse->setBlurRadius(20);
//    dse->setEnabled(true);
//    dse->setOffset(0, 0);
//    ui->top->setGraphicsEffect(dse);
...
void MainWindow::setupSignalsAndSlots() const {
    ...
    for (int i=0; i<widgetsCount; ++i) {
        ...
        if (auto&& tf = current->findChild<TestFrame::Ptr>(QString(), Qt::FindDirectChildrenOnly)) {
...
void MainWindow::setupUiLeftPanel() {
    ...
    MDCustomItem::Ptr fori = new MDCustomItem;
    fori->setObjectName(MAINWINDOW_MDCUSTOMITEM_FORI_OBJECT_NAME);
    fori->setText(MAINWINDOW_MDCUSTOMITEM_TEST_FORI);
